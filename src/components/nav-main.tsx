...
export function NavMain({
  ...
  return (
    <SidebarGroup>
      {label && (
        <SidebarGroupLabel className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[10px] px-3 mb-2">
          {label}
        </SidebarGroupLabel>
      )}
...
function NavCollapsibleItem({
  ...
  const isCurrentActive = isPathActive(item.url);
  const isChildActive =
    item.items?.some((sub: NavSubItem) => isPathActive(sub.url)) ?? false;
  const [open, setOpen] = React.useState(item.isActive || isChildActive);
  const [prevIsChildActive, setPrevIsChildActive] =
    React.useState(isChildActive);
            ...
            return (
              <SidebarMenuSubItem key={subItem.title}>
                ...
                >
                  <span
                    className={
                      isSubActive ? "text-orange-600 dark:text-orange-400" : ""
                    }
                  >
                    {subItem.title}
                  </span>
